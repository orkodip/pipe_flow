/*
NAVIER-STOKES SOLVER IN AXISYMMETRIC COORDINATES
CONVENTION USED ARE AS FOLLOWS
(R,Z)->(X,Y)->(i,j)->(I,J)
V=(V_r,V_z)->(u,v)
BOUNDARY CONDITIONS ARE AS FOLLOWS
LEFT BOUNDARY -> AXISYMMETRIC CONDITION = FREE SLIP AND NO PENETRATION
RIGHT BOUNDARY -> NO SLIP AND NO PENETRATION
BOTTOM BOUNDARY -> INLET (PRESCRIBED NORMAL VELOCITY)
TOP BOUNDARY -> OUTLET (PRESCRIBED PRESSURE)
...
RIGHT BOUNDARY -> DIRICHLET
BOTTOM BOUNDARY -> DIRICHLET
TOP BOUNDARY -> UPDATED FROM PRESSURE BC

PRESSURE BOUNDARY CONDITIONS
LEFT, RIGHT, AND BOTTOM BOUNDARY -> NEUMANN
...
void NS::vel_bc()
{
	for(int i=1;i<=I;i++)
	{
		v_NS[0][i]=1.0;	//bottom boundary
		u[0][i]=-u[1][i];
		v[0][i]=2.0-v[1][i];
		u_adv_NS[0][i]=0.0;
		v_adv_NS[0][i]=1.0;
		
		u[J+1][i]=u[J][i];	//top boundary
		v[J+1][i]=v[J][i];
		u_adv_NS[J][i]=u[J][i];
		v_adv_NS[J][i]=v[J][i];
	}
	...
	u[0][0]=-u[0][1]; u[J+1][0]=-u[J+1][1]; u[0][I+1]=-u[0][I]; u[J+1][I+1]=-u[J+1][I];
	v[0][0]=v[0][1]; v[J+1][0]=v[J+1][1]; v[0][I+1]=-v[0][I]; v[J+1][I+1]=-v[J+1][I];
}
void NS::P_bc()
{
	...
	for(int j=1;j<=J;j++)	//calculate advection value
	{
		for(int i=1;i<=I;i++)
		{
			A_x[j][i]=rho*((Xm[i]*u_EW[j][i]*u_adv_EW[j][i]-Xm[i-1]*u_EW[j][i-1]*u_adv_EW[j][i-1])/(CX[i]*dx)
					+(v_NS[j][i]*u_adv_NS[j][i]-v_NS[j-1][i]*u_adv_NS[j-1][i])/dy);
			A_y[j][i]=rho*((Xm[i]*u_EW[j][i]*v_adv_EW[j][i]-Xm[i-1]*u_EW[j][i-1]*v_adv_EW[j][i-1])/(CX[i]*dx)
					+(v_NS[j][i]*v_adv_NS[j][i]-v_NS[j-1][i]*v_adv_NS[j-1][i])/dy);
		}
	}
}
void NS::diff()
	...
	for(int j=1;j<=J;j++)
	{
		for(int i=1;i<=I;i++)
		{
			D_x[j][i]=((Xm[i]*2.0*mu*(u[j][i+1]-u[j][i])/dx)
					...
					+(mu*((u[j+1][i]-u[j][i])/dy+0.25*(v[j][i+1]+v[j+1][i+1]-v[j][i-1]-v[j+1][i-1])/dx)
					-mu*((u[j][i]-u[j-1][i])/dy+0.25*(v[j][i+1]+v[j-1][i+1]-v[j][i-1]-v[j-1][i-1])/dx))/dy;
			D_y[j][i]=(Xm[i]*mu*(0.25*(u[j+1][i]+u[j+1][i+1]-u[j-1][i]-u[j-1][i+1])/dy+(v[j][i+1]-v[j][i])/dx)
					-Xm[i-1]*mu*(0.25*(u[j+1][i]+u[j+1][i-1]-u[j-1][i]-u[j-1][i-1])/dy+(v[j][i]-v[j][i-1])/dx))/(CX[i]*dx)
					+(2.0*mu*(v[j+1][i]-v[j][i])/dy
					-2.0*mu*(v[j][i]-v[j-1][i])/dy)/dy;
		}
	}
}
void NS::mom()
{
	...
	for(int j=1;j<=J;j++)	//explicit calculation
	{
		for(int i=1;i<=I;i++)
		{
			us[j][i]=u[j][i]+dt*(-A_x[j][i]+D_x[j][i]-2.0*mu*u[j][i]/(CX[i]*CX[i]))/rho;
			vs[j][i]=v[j][i]+dt*(-A_y[j][i]+D_y[j][i])/rho;
		}
	}
}
void NS::calc_fc()
{
	...
	for(int i=1;i<=I;i++) v_NS[J][i]=vs[J][i];	//outflow boundary
}
void NS::Press()
{
	if(COUNT==0)	//setup the equations in the first iteration
	{
		...
		for(int j=1;j<=J;j++)
		{
			for(int i=1;i<=I;i++)
			{
				X0[(j-1)*I+i-1]=0.0;	//initial guess
				b0[(j-1)*I+i-1]=rho*dx*dx/dt*((Xm[i]*u_EW[j][i]-Xm[i-1]*u_EW[j][i-1])/(CX[i]*dx)+(v_NS[j][i]-v_NS[j-1][i])/dy);
				alpha=0.5*dx/CX[i]; beta=pow((dx/dy),2.0);
...
void NS::update()
{
	...
	for(int j=1;j<=J;j++)
	{
		for(int i=1;i<=I;i++)
		{
			...
			v_NS[j][i]=v_NS[j][i]-rdy*(P[j+1][i]-P[j][i])/rho;
			u[j][i]=us[j][i]-0.5*rdx*((P[j][i+1]-P[j][i])/rho+(P[j][i]-P[j][i-1])/rho);
			v[j][i]=vs[j][i]-0.5*rdy*((P[j+1][i]-P[j][i])/rho+(P[j][i]-P[j-1][i])/rho);
		}
	}
}
void NS::continuity()
